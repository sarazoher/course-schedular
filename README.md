...
Seed the course catalog (required for now before DB migration step):
...
- Clicking **Solve** runs the MILP solver and saves a snapshot of the latest result.
- The saved result is stored as a `PlanSolution` row (one latest snapshot per plan).
- Opening **Schedule** shows the most recently saved solution (it does not re-solve automatically).


## Plan Settings (Solver Options)

These checkboxes control solver behavior:

- **Enforce prerequisite rules**
  - Uses catalog prerequisites when they can be resolved.
  - Unresolved/external prerequisite tokens become warnings (they do not block solving).

- **Enforce credit limits per semester**
  - When enabled, the solver enforces `Max credits per semester`.

- **Prefer earlier completion when multiple schedules are valid**
  - When enabled, the solver first minimizes the *latest semester used* (finish ASAP),
    then packs courses earlier within that horizon.
  - When disabled, the solver simply packs courses earlier overall.


## Maintenance / Repair Scripts (Optional)

If a plan was created before the current default-offerings logic, some courses may have incorrect semester offerings.

### Regenerate default offerings for a plan
```bash
...
- **Corequisites are display-only**
  - Corequisite text is shown in the UI for context, but is not enforced by the solver.

- **Unresolved prerequisites become warnings**
  - If the prerequisite parser cannot resolve a token to a catalog course (or the prereq is classified as external),
    the solver records a warning and continues (it does not block scheduling).

- **Offerings repair may be needed for older plans**
  - Plans created before the current default-offerings logic may have incorrect offerings saved in the database.
  - Use `scripts/regen_offerings.py <plan_id>` to regenerate offerings for a plan.
